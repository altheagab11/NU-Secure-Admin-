...
<body>
	<div class="layout">
		...
		<main class="main">
			...
			<div class="toolbar">
				...
				<div class="d-flex flex-wrap gap-2">
					<a href="{{ route('admin.date-range-reports') }}" class="btn btn-outline-primary">
						<i class="bi bi-calendar-range me-1"></i> Generate Date-Range Report
					</a>
			...
			<div class="filters-card">
				...
				<form method="GET" action="{{ route('admin.daily-reports') }}">
					<div class="filters-row">
						<div>
							<label class="field-label" for="report_date">Report Date</label>
							<input type="date" id="report_date" name="report_date" class="filter-input" max="{{ $maxDate }}" value="{{ $filters['report_date'] }}">
						</div>
						<div>
						...
						<a href="{{ route('admin.daily-reports') }}" class="btn btn-outline-secondary">Reset</a>
					</div>
				</form>
			</div>
			...
			<div class="table-card">
				...
				@else
							...
							<tbody>
								@foreach ($reports as $report)
									@php
										...
										if ($generatorName === '') {
											$generatorName = $report->generated_by ? 'User #'.$report->generated_by : 'System Scheduler';
										}
										...
										<td>
											<div class="action-btns">
												@if ($report->isDownloadable())
													<a href="{{ route('admin.daily-reports.download', $report->id) }}" class="btn btn-sm btn-success">
														<i class="bi bi-download"></i> Download
													</a>
												@else
													<button type="button" class="btn btn-sm btn-outline-secondary" disabled title="Report is not ready for download">
														<i class="bi bi-download"></i> Download
													</button>
												@endif
												...
												<button
													...
													data-bs-target="#regenerateModal{{ $report->id }}"
												>
													<i class="bi bi-arrow-repeat"></i> Regenerate
												</button>
											</div>

											<div class="modal fade" id="regenerateModal{{ $report->id }}" tabindex="-1" aria-hidden="true">
												<div class="modal-dialog">
													<form method="POST" action="{{ route('admin.daily-reports.regenerate', $report->id) }}" class="modal-content">
														@csrf
														...
														<div class="modal-body">
															<p class="mb-2">
																This will replace the existing Excel file for
																<strong>{{ optional($report->report_date)->format('M d, Y') }}</strong>
																using the latest visitor data.
															</p>
															<div class="form-check">
																<input class="form-check-input" type="checkbox" value="1" id="confirm{{ $report->id }}" name="confirm" required>
																<label class="form-check-label" for="confirm{{ $report->id }}">
																	I confirm I want to replace the existing report.
																</label>
															</div>
														</div>
					...
					@include('admin.partials.table-pagination', [
						...
						'ariaLabel' => 'Daily reports pagination',
					])
				@endif
			</div>
		</main>
	...
	<div class="modal fade" id="generateReportModal" tabindex="-1" aria-labelledby="generateReportModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<form method="POST" action="{{ route('admin.daily-reports.generate') }}" class="modal-content" id="generateReportForm">
				@csrf
				<div class="modal-header">
					<h5 class="modal-title" id="generateReportModalLabel">Generate Daily Report</h5>
					<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<div class="modal-body">
					<label class="field-label" for="generate_report_date">Select Report Date</label>
					<input
						type="date"
						id="generate_report_date"
						name="report_date"
						class="filter-input"
						max="{{ $maxDate }}"
						value="{{ old('report_date', $maxDate) }}"
						required
					>
					<p class="small text-muted mt-2 mb-0">
						Future dates are not allowed. If a report already exists for the selected date, the existing file will be reused unless you regenerate it.
					</p>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
					<button type="submit" class="btn btn-primary" id="generateReportSubmit">
						<span class="generate-label">Generate Report</span>
	...
	<script>
		...
		(function () {
			...
			const submitBtn = document.getElementById('generateReportSubmit');
			if (form && submitBtn) {
